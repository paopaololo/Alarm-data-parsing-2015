...
def ParseAlarmsFromTag(TagData): # get alarms from tag data; return list of dictionaries per alarm in the tag
	...
	LineNo = 0
	# CommonAlarmData['Alarm Source'] = GetDataFromLine(TagData[LineNo]) #redundant 
	LineNo += 1
	ATagFound = False
	CommonAlarmData['Tag'] = GetDataFromLine(TagData[LineNo])
	ATagFound = GetDataFromLine(TagData[LineNo]).startswith('A', 4) or GetDataFromLine(TagData[LineNo]).startswith('A', 5) # checking if the data is '612XA or PDA'
	LineNo += 1
	...
	if ATagFound:
		Service = GetDataFromLine(TagData[LineNo]).strip('-').strip()
		...
		if NextLine3.isdigit(): # it's the second pattern
			Service = NextLine1 + ' ' + NextLine2
			EULow = NextLine3
			EUHigh = GetDataFromLine(TagData[LineNo + 3])
			EU = GetDataFromLine(TagData[LineNo + 4])
			LineNo += 5
		else: # it's the first pattern
			Service = NextLine1
			# get EU Low from the end of Service, by chopping off digits and - signs
	...
	if ATagFound:
		LineNo += 1 # For Tags with 612XA or PDA, only skipping one line
	else: LineNo += 2
	CommonAlarmData['Conseq no action'] = GetDataFromLine(TagData[LineNo])
	LineNo += 2 # skip next line
	CommonAlarmData['User3'] = GetDataFromLine(TagData[LineNo]) # time required for operator response
	LineNo += 1
	HSetpoint = GetDataFromLine(TagData[LineNo])
	LineNo += 2 # skip next line
