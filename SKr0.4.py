...
def SplitIntoTags(Data): # split Data into a list of separate lists, each for one tag
    ...
    def NextLineWithTag(Data, StartLine): # return index of next item in Data after item StartLine that starts with '612', or -1 if none found
        ...
        while (Index < len(Data)) and not TagFound:
            # find lines starting with '612' but not '612 '
            TagFound = Data[Index].strip().startswith('612') and (Data[Index].strip()[3] <> (' '))
            Index += 1
    ...
    while TagFound:
        ...
        NextTagLine = NextLineWithTag(Data, TagLine + 9) # find next tag line, assuming each tag uses at least 9 lines
...
def ParseAlarmsFromTag(TagData): # get alarms from tag data; return list of dictionaries per alarm in the tag
    ...
    def GetDataFromLine(Line, Keep3Dashes=False): # return data from str provided
        # if Keep3Dashes, lines containing only --- will be returned as-is; else will return ''
    ...
    def GetNumberFromEndOfLine(Line): # return string from end of Line that contains only digits and - or .
        if not ('x' + Line)[-1].isdigit(): return '' # exit immediately if Line has no digit at the end
        DigitFound = True
        Index = len(Line) - 1
    ...
    # check for case when AlarmTag has EU on the end; if so, all other lines down to EU are missing
    AlarmTagBits = AlarmTag[AlarmTag.index('-'):].split()
    if len(AlarmTagBits) > 1:
        if AlarmTagBits[1].strip():
            EU = AlarmTagBits[1].strip()
            AlarmTag = (AlarmTag[:AlarmTag.index(EU)]).strip() # chop the EU off the end of the tag
            Service = EULow = EUHigh = ''
            LineNo += 0
            SkipBlock1 = True
        else: SkipBlock1 = False
    else: SkipBlock1 = False
    ...
    if not SkipBlock1:
        # Now analyse block 1
        # next lines will be one of the following variations:
        # Var1: Service + EU Low + EU High, EU
        # Var2: Service 1, Service 2, EU Low (detect by: 3rd line is digits)
        # Var3: Service 1, Service 2, EU (if EU Low and EU High are missing) (detect by: 4th and 5th lines are '---')
        # Var4: Service 1 + EU Low, EU High, EU (detect by: 1st and 2nd lines end with a number, and 4th line starts with ---)
        # Var5: Service 1, EU
        ...
        NextLine3WithDashes = GetDataFromLine(TagData[LineNo + 2], Keep3Dashes=True)
        NextLine4 = GetDataFromLine(TagData[LineNo + 3]).strip()
        NextLine4WithDashes = GetDataFromLine(TagData[LineNo + 3], Keep3Dashes=True)
        NextLine5 = GetDataFromLine(TagData[LineNo + 4]).strip()
        NextLine5WithDashes = GetDataFromLine(TagData[LineNo + 4], Keep3Dashes=True)
        if NextLine3 == '' and ATagFound: # Paul added this option
            ...
            LineNo += 2
            #print "Var9"
        elif GetNumberFromEndOfLine(NextLine1) and GetNumberFromEndOfLine(NextLine2) and (NextLine4WithDashes[:3] == '---'): # Var4 found
            #print "Next lines 1, 2: '%s' '%s'" % (GetNumberFromEndOfLine(NextLine1), GetNumberFromEndOfLine(NextLine2))
            EULow = GetNumberFromEndOfLine(NextLine1)
            Service = NextLine1[:NextLine1.index(EULow)].strip() # remove EULow from end of Service
            EUHigh = NextLine2
            EU = NextLine3
            LineNo += 3
            #print "Var4"
        elif NextLine3WithDashes == '---' and NextLine4WithDashes == '---': # Var5 found
            Service = NextLine1
            EULow = '0'
            EUHigh = '100'
            EU = NextLine2
            LineNo += 2
            #print "Var5"
        elif NextLine4WithDashes == '---' and NextLine5WithDashes == '---': # Var3 found
            Service = NextLine1 + ' ' + NextLine2
            EULow = '0'
            EUHigh = '100'
            EU = NextLine3
            LineNo += 3
            #print "Var3"
        elif NextLine3.isdigit(): # Var2 found
            Service = NextLine1 + ' ' + NextLine2
            EULow = NextLine3
        ...
        #	LineNo += 3
            ...
            LineNo += 2
            #print "Var7"
        else: # Assume Var1 found. Get EU Low from the end of Service, by chopping off digits and - signs
            Service = NextLine1
    ...
    NextLineC2WithDashes = GetDataFromLine(TagData[LineNo + 1], Keep3Dashes=True).strip()
    NextLineC3 = GetDataFromLine(TagData[LineNo + 2]).strip()
    NextLineC4 = GetDataFromLine(TagData[LineNo + 3]).strip()
    NextLineC4WithDashes = GetDataFromLine(TagData[LineNo + 3], Keep3Dashes=True).strip()
    NextLineC5 = GetDataFromLine(TagData[LineNo + 4]).strip()
    NextLineC5WithDashes = GetDataFromLine(TagData[LineNo + 4], Keep3Dashes=True).strip()
    NextLineC6 = GetDataFromLine(TagData[LineNo + 5]).strip()
    NextLineC6WithDashes = GetDataFromLine(TagData[LineNo + 5], Keep3Dashes=True).strip()
    # print NextLineC2
    # print TagData

    if NextLineC1.startswith('PD612'): # multiple lines missing, this line is P&ID (Peter)
        Conseq = 'Missing in the input file provided by client.'
        Response = 'Missing in the input file provided by client.'
        HHSetpoint = HSetpoint = LSetpoint = LLSetpoint = ''
        LineNo -= 1 # skip back to line with normal operating limit etc
        # print "Found tag with multiple missing lines"
    else:
        LineC1Start = (NextLineC1.split() + [''])[0]
        if not (False in [(char in '1234567890.-') for char in LineC1Start]):
            # test if 1st line begins with a number or ---; if so, it's the H setpoint followed by conseq
            Conseq = NextLineC1[len(LineC1Start)+1:]
            Response = ''
            LineNo += 0
        elif NextLineC5.startswith('LESS') or NextLineC5.startswith('TBD') or NextLineC5.startswith('NONE'):
            # when conseq spanning across two lines, the next line will be response
            Conseq = NextLineC1 + ' ' + NextLineC2
            Response = NextLineC5
            LineNo += 5
            #print "AAA"
        elif NextLineC6.startswith('LESS') or NextLineC6.startswith('TBD') or NextLineC6.startswith('NONE'):
            # when conseq spanning across 3 lines, the next line will be response
            Conseq = NextLineC1 + ' ' + NextLineC2 + ' ' + NextLineC3
            Response = NextLineC6
            LineNo += 6
            ...
            LineNo += 3
            #print "NBB"
        elif NextLineC2.startswith('LESS') or NextLineC2.startswith('TBD') or NextLineC2.startswith('None'):
            # conseq on 1 line, operator response missing
            Conseq = NextLineC1
            Response = NextLineC2
            LineNo += 2
        elif NextLineC4WithDashes == '' and NextLineC5WithDashes == '' and NextLineC6WithDashes <> '':
            # 3 lines of consequence, 2 lines of operator response (6th line is a setpoint)
            Conseq = NextLineC1 + ' ' + NextLineC2 + ' ' + NextLineC3
            Response = ''
            LineNo += 5
        elif NextLineC2WithDashes == '': # one line of conseq, one line of response
            Conseq = NextLineC1
            Response = ''
            LineNo += 2
            #print "WWW"
            ...
            Conseq = 'Missing in the input file provided by client.'
            Response = 'Missing in the input file provided by client.'
        else: # if no other case found (Peter, for debug)
            Conseq = Response = '*** NOT FOUND ***'

        if AllmissingbutEU: HSetpoint = ''
        else: HSetpoint = (GetDataFromLine(TagData[LineNo]).strip().split() + [''])[0] # take line up to first space
...
#InputFileName = 'Alarm list raw 34911 only.txt'
